...
enum PageBridge {
    ...
    (function () {
      ...
      // 2026-09-04:詞庫「自動查找」功能用——取回頁面標題當漫畫名稱的
      // 搜尋起點。刻意不做任何清理(不剝站名/集數這類樣板文字),因為
      // 不同站的標題包裝格式差很多,寫死一套規則清不乾淨——原生端把這個
      // 當「起手值」塞進可編輯欄位,交給使用者自己修剪過再送出去查詢。
      window.imgTranslateGetPageTitle = function () {
