...
- 跟 Qwen 三方比對後,發現 TranslateGemma 預設語氣偏書面正式(「你很吵」),Qwen 更口語生動(「你吵死了」)。嘗試加一句語氣提示(要求口語自然、像漫畫台詞),套用同一套保險過濾機制。另外原文「...」拖長音收尾常被模型收成句號,加了一個確定性的後處理把它換回「……」保留語感。**語氣提示這項一樣是實驗性,TranslateGemma 是翻譯專用模型,服從度沒把握,需要裝機驗證**
